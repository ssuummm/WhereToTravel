...
def create_map(data):
    ...
    <div style="position: fixed;
                 top: 10px; right: 10px; width: auto; padding:0 10px; max-height: 400px; overflow-y: auto;
...
if __name__ == "__main__":
    ...
    # 데이터프레임 인덱스 리셋
    filtered_out_gr.reset_index(drop=True, inplace=True)

    filtered_out_gr_reset = filtered_out_gr.reset_index(drop=False)
    filtered_out_gr_reset.index += 1

    # 데이터프레임 테이블로 보여주기
    st.dataframe(filtered_out_gr_reset[['관광지명', '주소', '분류']])

    # 지도 생성 및 렌더링
    map = create_map(filtered_out_gr_reset)
